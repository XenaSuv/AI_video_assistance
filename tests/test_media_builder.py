...
class TestBuildVoice:
    ...
    def test_audio_dir_missing_calls_asyncio_run(self, tmp_path):
        """Audio dir doesn't exist → TTS + image fetch should be triggered."""
        cp = MagicMock()
        cp.is_done.return_value = False
    ...
    def test_fewer_mp3s_than_scenes_calls_asyncio_run(self, tmp_path):
        """Audio dir exists but has fewer mp3s than scenes → TTS needed."""
    ...
    def test_cached_mp3s_present_does_not_call_asyncio_run(self, tmp_path):
        """Enough mp3s exist → reuse cache, no TTS call."""
        audio_dir = tmp_path / "audio"
        audio_dir.mkdir()
        for i in range(3):
    ...
    def test_sets_total_duration_sec_in_summary(self, tmp_path):
        cp = MagicMock()
        cp.is_done.return_value = True
        script = _mock_script(3)  # each scene duration_sec=30
        summary = {}
        ...
        assert summary["total_duration_sec"] == 90  # 3 × 30
...
class TestBuildSubtitles:
    ...
    def test_en_intro_none_when_file_missing(self, tmp_path):
        ms = self._patched_settings(tmp_path)
        # ai-news-intro.mp4 does NOT exist
        cp = MagicMock()
        cp.is_done.return_value = True
        b = _builder(tmp_path, cp=cp)
    ...
    def test_en_outro_always_set(self, tmp_path):
        ms = self._patched_settings(tmp_path)
...
class TestBuildVideo:
    ...
    def test_needs_rebuild_even_when_done(self, tmp_path):
        cp = MagicMock()
        cp.is_done.return_value = True
    ...
    def test_marks_seen_and_marks_done_on_build(self, tmp_path):
        cp = MagicMock()
        cp.is_done.return_value = False
        seen = MagicMock()
...
class TestBuildThumbnail:
    ...
    def test_not_done_generates_thumbnail(self, tmp_path):
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(
                patch("src.media_builder.pick_thumbnail", return_value=fake_thumb)
            )
            stack.enter_context(patch("src.media_builder.settings"))
            b.build_thumbnail()
    ...
    def test_done_but_missing_file_regenerates(self, tmp_path):
        """Checkpoint done but thumbnail file missing → should regenerate."""
        b, cp = self._prep(tmp_path, cp_done=True, style="bold")
        # thumbnail_bold.jpg does NOT exist
        fake_thumb = tmp_path / "thumbnail_bold.jpg"
    ...
    def test_done_sets_thumbnail_style_from_checkpoint_metadata(self, tmp_path):
